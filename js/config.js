...
(function () {
  ...
  Game.CONST = {
    ...
    // 每把武器各管自己扇形宽的一块，**扇形圆心是玩家**（半径 = 近战射程）。
    // 总覆盖 = 武器数 × 扇形宽，所以武器越多打得的面越广：一把 135°，两把 270°，
    // 六把时已经满圈还有重叠。不能再按 360°/武器数均分 —— 那样覆盖面恒定 360°，
    // 加武器只加输出，玩家感知不到手里多了什么。
    // 圆心是玩家而不是武器 —— 以武器为圆心时贴着玩家的敌人落在武器背后，
    // 内环永远打不到（用户 2026-09-25「现在内环的攻击攻击不到」）。
    // 每把武器的扇形宽就是 WEAPONS 表里的 arc（冻结值，近战用它、远程没有）。
    // 远程表里 arc 是 0，单独给一个，取和铁剑一样的 135° 让手感一致。
