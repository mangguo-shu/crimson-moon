...
(function () {
  ...
  /* ---------------- 常量 ---------------- */
  Game.CONST = {
    // 改完打包后先看主菜单右下角这个号对不对 —— 2026-09-26 真机验收时用户手机上
    ...
    // —— 环绕轨道：多把武器挂在玩家身边，按槽位序号等分角度，整圈慢速旋转 ——
    WEAPON_ORBIT_R: 62,               // 轨道半径。取玩家半径(14)到首个敌人接触距离之间
    WEAPON_ORBIT_SPEED: 0.25,         // 轨道转速 rad/s（整圈 25s），慢一点才有「跟随」感
    WEAPON_ORBIT_OFFSET: -Math.PI / 2,  // 第一把武器的基准角（−π/2 = 玩家正上方）
    // 索敌**不做角度过滤**：剑转到哪儿不再决定能不能打，永远打射程内最近的那个
    // （用户 2026-09-26「怪物在上方，即使剑转到了下方，也可以攻击上方的怪物，
    // 默认攻击距离最近的怪物」）。圆心是玩家而不是武器 —— 以武器为圆心时贴着
    // 玩家的敌人落在武器背后，内环永远打不到（「现在内环的攻击攻击不到」）。
    // WEAPON_ARC 只是挥砍扇形的宽度（画刀光用），取 360°/6 = 60°，让六把武器
    // 正好排满一圈不重叠（「覆盖面可以缩短，要求可以容纳六把武器」）。
    // WEAPONS 表里的 arc 字段现在完全不作数，只是数据留用。
    WEAPON_ARC: Math.PI / 3,
