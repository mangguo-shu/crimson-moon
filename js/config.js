...
(function () {
  ...
  Game.CONST = {
    ...
    // —— 环绕布置：多把武器固定在玩家身边，按槽位序号等分角度，不转圈 ——
    WEAPON_ORBIT_R: 62,              // 布置半径。取玩家半径(14)到首个敌人接触距离之间
    WEAPON_ORBIT_OFFSET: -Math.PI / 2,  // 第一把武器的基准角（−π/2 = 玩家正上方）
    // 每把武器负责 360° / 武器数 的一块扇形，**扇形圆心是玩家**（半径 = 近战射程）。
    // 武器只是「这块扇形归谁」的标记 —— 之前以武器为圆心时，贴着玩家的敌人
    // 落在所有武器的背后，谁都打不到（用户 2026-09-25「现在内环的攻击攻击不到」）。
    // WEAPONS 表里的 arc 字段本意就是扇形宽，但改成 360°/N 之后它不再决定索敌，
    // 表本体不动，只留作数据。
